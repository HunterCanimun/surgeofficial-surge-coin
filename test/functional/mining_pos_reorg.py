...
class ReorgStakeTest(SurgeTestFramework):
    ...
    def check_money_supply(self, expected_srg):
        # verify that nodes have the expected SRG supply
        srg_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(srg_supply, [DecimalAmt(expected_srg)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that SRG supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check SRG supply...")
