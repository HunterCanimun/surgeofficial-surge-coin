...
"""Test surged with different proxy configuration.

Test plan:
- Start surged's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on surged side:
...
- Create surgeds that connect to them
- Manipulate the surgeds using addnode (onetry) an observe effects
...
class ProxyTest(SurgeTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: surged's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
