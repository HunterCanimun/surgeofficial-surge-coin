...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SURGEParams"
    else
        PARAMS_DIR="$HOME/.surge-params"
    fi
fi
...
Failed to fetch the SURGE zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
SURGE - fetch-params.sh

This script will fetch the SURGE zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SURGE zkSNARK parameters. Note that it is
