...
#### Sapling Parameters
In order to run, SURGE Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run SURGE v5.0.0:
...
* Top bar showing shield SRG balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield SRG balance.
...
| `getsupplyinfo` | Returns detailed SRG supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in SRG
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in SRG
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zSRGsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield SRG)
  The `moneysupply` JSON return entry is the total SRG supply (the sum of all transparent and shield SRG)
...
| `getaccountaddress` | Returned the current SURGE address associated with an internal account | Account system removal |
| `setaccount` | Associate a SURGE address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given SURGE address | Account system removal |
| `getaddressesbyaccount` | Returned a list of SURGE addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a SURGE address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a SURGE address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zSRG balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zSRG mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zSRG mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zSRG amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zSRG to a SRG address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zSRG mints to a SRG address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zSRGs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzsrgseed` | Set the wallet's deterministic zSRG seed to a specific value | Zerocoin deprecation |
| `getzsrgseed` | Return the current deterministic zSRG seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zSRG seed | Zerocoin deprecation |
| `dzsrgstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzsrg` | Do a search for deterministically generated zSRG that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_surge unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake surged and unit tests: fixing missing MacOS framework. (furszy)
