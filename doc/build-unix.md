UNIX BUILD NOTES
====================
Some notes on how to build SURGE Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile SURGE Core and the dependencies,
...
This will build surge-qt as well, if the dependencies are met.
...
memory available when compiling SURGE Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~surge/+archive/surge).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:surge/surge
    sudo apt-get update
...
To build SURGE Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build surge-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a surge-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip surged" to strip the debug
...
Security
--------
To help make your SURGE Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, SURGE Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, SURGE Core may be compiled in
