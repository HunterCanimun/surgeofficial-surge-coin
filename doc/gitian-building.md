...
*Setup instructions for a gitian build of SURGE Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SURGE
...
to the SURGE GitHub release page.
...
- [Building SURGE Core](#building-surge-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/surge-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/surge-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/surge-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/surge-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/surge-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/surge-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/surge-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@surge.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/surge-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/surge-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/surge-osx-*-build.assert
```
