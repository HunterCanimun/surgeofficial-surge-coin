...
This document provides guidelines for internationalization of the SURGE Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to SURGE network</source>
        <translation>
            <numerusform>%n active connection to SURGE network</numerusform>
            <numerusform>%n active connections to SURGE network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `surge_en.ts` remains unchanged.
