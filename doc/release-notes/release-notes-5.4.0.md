SURGE Core version v5.4.0 is now available from: <https://github.com/surge-project/surge/releases>
...
If you are running an older version, shut it down. Wait until it has completely shut down, then run the installer (on Windows) or just copy over /Applications/SURGE-Qt (on Mac) or surged/surge-qt (on Linux).
...
The `autocombinerewards` RPC command was soft-deprecated in v5.3.0 and replaced with explicit setter/getter commands `setautocombinethreshold`/`getautocombinethreshold`. SURGE Core, by default, will no longer accept the `autocombinerewards` command, returning a deprecation error, unless the `surged`/`surge-qt` is started with the `-deprecatedrpc=autocombinerewards` option.
...
- #2579 surge-cli better error handling + libevent RAII upstream backports (furszy)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/surge-project-translations/), and every single person who tested the release candidates.
