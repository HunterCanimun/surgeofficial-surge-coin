...
CSrgStake* CSrgStake::NewSrgStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CSrgStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CSrgStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CSrgStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CSrgStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CSrgStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CSrgStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CSrgStake::CreateTxOuts(const CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal) const
{
...
CDataStream CSrgStake::GetUniqueness() const
{
    //The unique identifier for a SRG stake is the outpoint
...
const CBlockIndex* CSrgStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CSrgStake: uninitialized pindexFrom");
    return pindexFrom;
}
