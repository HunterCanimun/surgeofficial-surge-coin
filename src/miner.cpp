...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("SURGEMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("SURGEMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("SURGEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("surge-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SURGEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("SURGEMiner exception");
    } catch (...) {
        LogPrintf("SURGEMiner exception");
    }

    LogPrintf("SURGEMiner exiting\n");
}
