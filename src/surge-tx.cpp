...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " surge-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  surge-tx [options] <hex-tx> [commands]  Update hex-encoded surge transaction\n" +
                               "or:     surge-tx [options] -create [commands]   Create hex-encoded surge transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded surge transaction
