...
UniValue getpeerinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"subver\": \"/Surge Core:x.x.x.x/\",  (string) The string version\n"
...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("addnode", "\"192.168.0.6:6120\" \"onetry\"") + HelpExampleRpc("addnode", "\"192.168.0.6:6120\", \"onetry\""));
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:6120\",  (string) The surge server IP and port we're connected to\n"
...
UniValue getnetworkinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"subversion\": \"/Surge Core:x.x.x.x/\",     (string) the server subversion string\n"
...
static UniValue addpeeraddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addpeeraddress", "\"1.2.3.4\" 6120")
            + HelpExampleRpc("addpeeraddress", "\"1.2.3.4\", 6120"));
    }
    if (!g_connman) {
        throw JSONRPCError(RPC_CLIENT_P2P_DISABLED, "Error: Peer-to-peer functionality missing or disabled");
    }
