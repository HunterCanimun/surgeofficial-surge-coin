...
static const CBlockIndex* FindIndexFrom(uint32_t nChecksum, libzerocoin::CoinDenomination denom, int cpHeight)
{
    ...
    // Height limits are ensured by the contextual checks in NewZSrgStake
...
CLegacyZSrgStake* CLegacyZSrgStake::NewZSrgStake(const CTxIn& txin, int nHeight)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend()) {
        LogPrintf("%s: unable to initialize CLegacyZSrgStake from non zc-spend", __func__);
        return nullptr;
    }
    ...
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) ||
        ...
        LogPrint(BCLog::LEGACYZC, "%s : zSRG stake block: height %d outside range", __func__, nHeight);
        return nullptr;
    }
    ...
    if (_pindexFrom == nullptr) {
        LogPrintf("%s : Failed to find the block index for zsrg stake origin", __func__);
        return nullptr;
    }

    // All good
    return new CLegacyZSrgStake(_pindexFrom, _nChecksum, _denom, _hashSerial);
}
