...
bool CLegacyZSrgStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZSrgStake from non zc-spend", __func__);
    ...
    *this = CLegacyZSrgStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zsrg stake origin", __func__);

    // All good
    return true;
}

CLegacyZSrgStake::CLegacyZSrgStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZSrgStake::GetIndexFrom() const
{
...
CAmount CLegacyZSrgStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZSrgStake::GetUniqueness() const
{
...
bool CLegacyZSrgStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zSRG stake block: height %d outside range", __func__, nHeight);
