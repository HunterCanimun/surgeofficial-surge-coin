...
#include "qt/surge/topbar.h"
#include "qt/surge/forms/ui_topbar.h"
#include "qt/surge/lockunlock.h"
#include "qt/surge/qtutils.h"
#include "qt/surge/receivedialog.h"
#include "qt/surge/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/surge/balancebubble.h"
...
TopBar::TopBar(SURGEGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopSrg, ui->labelAmountTopShieldedSrg}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSrg}, "amount-topbar");
    setCssProperty({ui->labelPendingSrg, ui->labelImmatureSrg}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SURGE")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // SRG Total
    QString totalSrg = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // SRG
    // Top
    ui->labelAmountTopSrg->setText(totalTransparent);
    ui->labelAmountTopShieldedSrg->setText(totalShielded);
    // Expanded
    ui->labelAmountSrg->setText(totalSrg);
    ui->labelPendingSrg->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureSrg->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
