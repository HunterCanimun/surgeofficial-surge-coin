...
#include "qt/surge/settings/settingsfaqwidget.h"
#include "qt/surge/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/surge/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(SURGEGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableSRG,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableSRG,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("SURGE is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("SURGE utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableSRGContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received SURGE requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your SURGE wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableSRG->setText(unspendableSRGContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of SURGE with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("surge-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.surgeofficial.uk'>" + tr("#support in our Discord") + "</a>.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node SURGE core wallet with a "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in SURGE Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the SURGE network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("85,000 SRG per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 85,000 SRG collateral "
    ...
    connect(ui->pushButton_UnspendableSRG, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableSRG);});
    ...
    if (parent)
        connect(parent, &SURGEGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
