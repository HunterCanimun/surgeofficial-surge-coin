...
#include "qt/surge/settings/forms/ui_settingsinformationwidget.h"
...
SettingsInformationWidget::SettingsInformationWidget(SURGEGUI* _window,QWidget *parent) :
    ...
    ui(new Ui::SettingsInformationWidget)
{
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this](){
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open surge.conf with default application"));
    });
