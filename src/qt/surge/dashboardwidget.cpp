...
#include "qt/surge/dashboardwidget.h"
#include "qt/surge/forms/ui_dashboardwidget.h"
#include "qt/surge/sendconfirmdialog.h"
#include "qt/surge/txrow.h"
...
DashboardWidget::DashboardWidget(SURGEGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareSrg, "square-chart-srg");
    setCssProperty(ui->labelSquarezSrg, "square-chart-zsrg");
    setCssProperty(ui->labelSrg, "text-chart-srg");
    setCssProperty(ui->labelZsrg, "text-chart-zsrg");
    ...
    setCssProperty(ui->labelAmountSrg, "text-stake-srg-disable");
    setCssProperty(ui->labelAmountZsrg, "text-stake-zsrg-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &SURGEGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SRG")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isSrg = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZSRG;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair SRG, zSRG
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            srg = (pair.first != 0) ? pair.first / 100000000 : 0;
            zsrg = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalSrg += pair.first;
            chartData->totalZsrg += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSrg.append(srg);
        chartData->valueszSrg.append(zsrg);

        int max = std::max(srg, zsrg);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(14,96,182));
    set1->setColor(QColor(84,158,241));
    ...
    set0->append(chartData->valuesSrg);
    set1->append(chartData->valueszSrg);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalSrg > 0 || chartData->totalZsrg > 0) {
        setCssProperty(ui->labelAmountSrg, "text-stake-srg");
        setCssProperty(ui->labelAmountZsrg, "text-stake-zsrg");
    } else {
        setCssProperty(ui->labelAmountSrg, "text-stake-srg-disable");
        setCssProperty(ui->labelAmountZsrg, "text-stake-zsrg-disable");
    }
    forceUpdateStyle({ui->labelAmountSrg, ui->labelAmountZsrg});
    ui->labelAmountSrg->setText(GUIUtil::formatBalance(chartData->totalSrg, nDisplayUnit));
    ui->labelAmountZsrg->setText(GUIUtil::formatBalance(chartData->totalZsrg, nDisplayUnit, true));
