...
#include "qt/surge/send.h"
#include "qt/surge/forms/ui_send.h"
#include "qt/surge/addnewcontactdialog.h"
#include "qt/surge/qtutils.h"
#include "qt/surge/sendchangeaddressdialog.h"
#include "qt/surge/optionbutton.h"
#include "qt/surge/sendconfirmdialog.h"
#include "qt/surge/guitransactionsutils.h"
#include "qt/surge/loadingdialog.h"
...
SendWidget::SendWidget(SURGEGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a SURGE URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-srg");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onSRGSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onSRGSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent SRGs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield SRGs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent SRGs to shield."));
    }
}
...
void SendWidget::onSRGSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto srgAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(srgAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(srgAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(srgAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
