...
#include "qt/surge/forms/ui_addresseswidget.h"
#include "qt/surge/addressfilterproxymodel.h"
#include "qt/surge/addresslabelrow.h"
#include "qt/surge/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(SURGEGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto srgAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(srgAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(srgAdd);
        if (walletModel->updateAddressBookLabels(srgAdd, label.toUtf8().constData(),
