...
#include "qt/surge/forms/ui_snackbar.h"
#include "qt/surge/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SURGEGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SURGEGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
