...
#include "qt/surge/qtutils.h"
#include "qt/surge/defaultdialog.h"
#include "shutdown.h"
...
const QString SURGEGUI::DEFAULT_WALLET = "~Default";

SURGEGUI::SURGEGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void SURGEGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &SURGEGUI::toggleHidden);
...
void SURGEGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &SURGEGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &SURGEGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &SURGEGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &SURGEGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &SURGEGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &SURGEGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &SURGEGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &SURGEGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &SURGEGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &SURGEGUI::execDialog);
    connect(governancewidget, &GovernanceWidget::showHide, this, &SURGEGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &SURGEGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &SURGEGUI::execDialog);
}


void SURGEGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void SURGEGUI::handleRestart(QStringList args)
{
...
void SURGEGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &SURGEGUI::message);
...
void SURGEGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &SURGEGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &SURGEGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void SURGEGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void SURGEGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void SURGEGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &SURGEGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &SURGEGUI::show);
                e->ignore();
            }
        }
    }
...
void SURGEGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void SURGEGUI::messageInfo(const QString& text)
{
...
void SURGEGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool SURGEGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void SURGEGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void SURGEGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SURGEGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void SURGEGUI::goToSend()
{
    showTop(sendWidget);
}

void SURGEGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void SURGEGUI::goToMasterNodes()
{
    showTop(masterNodesWidget);
}

void SURGEGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void SURGEGUI::goToGovernance()
{
    showTop(governancewidget);
}

void SURGEGUI::goToSettings(){
    showTop(settingsWidget);
}

void SURGEGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void SURGEGUI::goToReceive()
{
    showTop(receiveWidget);
}

void SURGEGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void SURGEGUI::showTop(QWidget* view)
{
...
void SURGEGUI::changeTheme(bool isLightTheme)
{
...
void SURGEGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool SURGEGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void SURGEGUI::showHide(bool show)
{
...
int SURGEGUI::getNavWidth()
{
    return this->navMenu->width();
}

void SURGEGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void SURGEGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void SURGEGUI::setMNModel(MNModel* mnModel)
{
    if (!stackedContainer || !clientModel) return;
...
bool SURGEGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &SURGEGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &SURGEGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &SURGEGUI::message);
    connect(topBar, &TopBar::message, this, &SURGEGUI::message);
    connect(sendWidget, &SendWidget::message,this, &SURGEGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &SURGEGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &SURGEGUI::message);
    connect(governancewidget, &GovernanceWidget::message,this, &SURGEGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &SURGEGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &SURGEGUI::incomingTransaction);

    return true;
}

bool SURGEGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void SURGEGUI::removeAllWallets()
{
    // Single wallet supported.
}

void SURGEGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(SURGEGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SURGEGUI::subscribeToCoreSignals()
{
...
void SURGEGUI::unsubscribeFromCoreSignals()
{
