...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::SRG);
    ...
    if (settings.contains("nAnonymizeSurgeAmount") || reset)
        gArgs.SoftSetArg("-anonymizesurgeamount", settings.value("nAnonymizeSurgeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in surge.cpp during shotdown to also remove the window positions
