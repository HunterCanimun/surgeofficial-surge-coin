    ...
    for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned SRGs, op_return could be for a proposal/budget fee or another sort of data stored there.
...
bool IsZSRGType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZSRG:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zSrg:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZSRG ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZSRG);
}
