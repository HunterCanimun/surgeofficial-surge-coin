...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select SRG Outputs to Spend" :
            "Select Shielded SRG to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::SRG;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show SURGE address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
