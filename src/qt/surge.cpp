...
#ifdef ENABLE_WALLET
#include "qt/surge/governancemodel.h"
#include "qt/surge/mnmodel.h"
...
{
    window = new SURGEGUI(networkStyle, nullptr);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. SURGE can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data and blocks directory and parse surge.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", SURGE_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
