...
#include "qt/surge/splash.h"
#include "qt/surge/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("surge-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in surge.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in surge.qrc)
...
/** Class encapsulating SURGE Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (SURGEGUI)
...
{
    window = new SURGEGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &SURGEGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &SURGEGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(SURGEGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SURGEGUI::DEFAULT_WALLET);
        ...
        // SURGE: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SURGEGUI::handlePaymentRequest);
        connect(window, &SURGEGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. SURGE can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(surge_locale);
    Q_INIT_RESOURCE(surge);
    ...
    /// 6. Determine availability of data directory and parse surge.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("SURGE Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("SURGE Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // surge: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("SURGE Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
