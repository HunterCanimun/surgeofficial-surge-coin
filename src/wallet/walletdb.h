...
public:
    ...
    bool WriteZSRGSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZSRGSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZSRGSeed_deprecated(uint256& seed);
    bool EraseZSRGSeed();
    bool EraseZSRGSeed_deprecated();

    bool WriteZSRGCount(const uint32_t& nCount);
    bool ReadZSRGCount(uint32_t& nCount);
