...
const char * const SURGE_CONF_FILENAME = "surge.conf";
const char * const SURGE_PID_FILENAME = "surge.pid";
const char * const SURGE_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SURGEParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SURGEParams
    // Mac: ~/Library/Application Support/SURGEParams
    // Unix: ~/.surge-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SURGEParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SURGEParams";
#else
    // Unix
    return pathRet / ".surge-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/surge/sapling-spend.params";
        sapling_output = "/usr/share/surge/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/surge/sapling-spend.params";
            sapling_output = "/usr/local/share/surge/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", SURGE_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", SURGE_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty surge.conf if it does not exist
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", SURGE_PID_FILENAME));
