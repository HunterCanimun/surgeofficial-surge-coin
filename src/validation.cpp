...
#include "warnings.h"
#include "zsrgchain.h"
#include "zsrg/zerocoin.h"
#include "zsrg/zsrgmodule.h"
...
#if defined(NDEBUG)
#error "SURGE cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usrg) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zSRG mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zSRG mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k SRG
    ...
    else if (nHeight <= 414600) {
        ret = COIN * 16;
    }
    else if (nHeight <= 599931) {
        ret = COIN * 14;
    }
    else if (nHeight <= 999931) {
        ret = COIN * 11;
    }
    else if (nHeight <= 1499931) {
        ret = COIN * 8;
    }
    else if (nHeight <= 2499931) {
        ret = COIN * 6;
    }
    else if (nHeight <= 5499931) {
        ret = COIN * 4;
    }
	else if (nHeight <= 9499935) {
        ret = COIN * 3;
    }
    else {
        ret = COIN * 0;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZSRGModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
{
    util::ThreadRename("surge-scriptch");
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZSRGModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const auto& txIn : block.vtx) {
        ...
        // double check that there are no double spent zSRG spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSRG serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Surge Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nSurgeBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nSurgeBadBlockBits)) {
            // accept SURGE block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> srgInputs;
        std::vector<CTxIn> zSRGInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zSRGInputs.push_back(stakeIn);
            }else{
                srgInputs.push_back(stakeIn);
            }
        }
        const bool hasSRGInputs = !srgInputs.empty();
        const bool hasZSRGInputs = !zSRGInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasSRGInputs) {
                    ...
                    for (const CTxIn& srgIn : srgInputs)
                        if(srgIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zSRG inputs.
            if(hasZSRGInputs) {
                for (const CTxIn& zSrgInput : zSRGInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zSrgInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsrg");
