                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zsrgActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZsrgActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZsrgActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zsrgActive || publicZsrgActive)) {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            ...
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsrg");
            }
        }
